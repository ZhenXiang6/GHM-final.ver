...
namespace Match3
{
    ...
    public class Board : MonoBehaviour
    {
        ...
        private Vector3Int m_SelectedGemOriginalCell;
        private Vector3Int m_PreviousCell;
        private bool m_IsDragging = false;
        private HashSet<Vector3Int> m_MovedCells = new HashSet<Vector3Int>();


        private float m_MoveTimeLimit = 5.0f; // 每次移动的时间限制，单位为秒，可以根据需要调整
        private float m_CurrentMoveTime = 0.0f;
        private bool m_IsTimingMove = false;

        public bool IsTimingMove => m_IsTimingMove;
        public float MoveTimeLimit => m_MoveTimeLimit;
        public float CurrentMoveTime => m_CurrentMoveTime;
        ...
        private void Update()
        {
            ...
            if (m_IsTimingMove)
            {
                ...
                if (m_CurrentMoveTime >= m_MoveTimeLimit)
                {
                    // 移动超时，强制结束移动
                    ForceEndMove();
                }
        ...
        private void ForceEndMove()
        {
            m_IsTimingMove = false;
            m_IsDragging = false;
            ...
            {
                // 确保珠子正确对齐到格子中心
                m_SelectedGem.transform.position = m_Grid.GetCellCenterWorld(m_PreviousCell);

                // 触发匹配检查，对已移动的所有格子进行检查
                foreach (var cell in m_MovedCells)
                {
                    m_CellToMatchCheck.Add(cell);
                }
        ...
        bool DoCheck(Vector3Int startCell, bool createMatch = true)
        {
            ...
            if (createMatch)
            {
                ...
                finalMatch.SpawnedBonus = null; //matchedBonusGem.Count == 0 ? null : matchedBonusGem[Random.Range(0, matchedBonusGem.Count)];
        ...
        void CheckInput()
        {
            ...
            var isTouching = GameManager.Instance.ClickAction.IsPressed();
            ...
            if (pressedThisFrame)
            {
                ...
                // 檢查該格子是否有可移動的珠子
                if (CellContent.ContainsKey(startCell) && CellContent[startCell].CanBeMoved)
                {
                    m_SelectedGem = CellContent[startCell].ContainingGem;
                    m_SelectedGemOriginalCell = startCell;
                    m_PreviousCell = startCell;
                    m_IsDragging = true;
                    m_IsTimingMove = true;
            ...
            else if (isTouching && m_IsDragging)
            {
                // 更新選中珠子的位置，使其跟隨指針移動
                if (m_SelectedGem != null)
                {
                    m_SelectedGem.transform.position = worldPos;

                    // 獲取當前指針所在的格子
                    var currentCell = m_Grid.WorldToCell(worldPos);
                    currentCell.z = 0;

                    // 如果當前格子與上一個格子不同，並且在棋盤範圍內
                    while (currentCell != m_PreviousCell && CellContent.ContainsKey(currentCell))
                    {
                        // 計算移動方向
                        int dx = currentCell.x - m_PreviousCell.x;
                        int dy = currentCell.y - m_PreviousCell.y;

                        // 正規化移動，一次只移動一格
                        int stepX = dx == 0 ? 0 : (dx > 0 ? 1 : -1);
                        int stepY = dy == 0 ? 0 : (dy > 0 ? 1 : -1);

                        Vector3Int nextCell = m_PreviousCell + new Vector3Int(stepX, stepY, 0);

                        // 與下一個格子的珠子交換
                        var targetGem = CellContent[nextCell].ContainingGem;

                        CellContent[m_PreviousCell].ContainingGem = targetGem;
                        if (targetGem != null)
                        {
                            targetGem.MoveTo(m_PreviousCell);
                            targetGem.transform.position = m_Grid.GetCellCenterWorld(m_PreviousCell);
                        }

                        CellContent[nextCell].ContainingGem = m_SelectedGem;
                        m_SelectedGem.MoveTo(nextCell);

                        // 將格子加入已移動格子集合
                        m_MovedCells.Add(m_PreviousCell);
                        m_MovedCells.Add(nextCell);

                        // 更新上一個格子
                        m_PreviousCell = nextCell;

                        // 重新計算當前格子，防止指針在一幀內移動多個格子
                        currentCell = m_Grid.WorldToCell(worldPos);
                        currentCell.z = 0;
                    }
                }
            }
